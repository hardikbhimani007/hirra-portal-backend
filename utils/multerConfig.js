    ...
    return async (req, res, next) => {
        try {
            ...
            if (base64String.startsWith('data:image')) {
                ...
                req.body.profile_pictures = `/${userFolder}/${filename}`.replace(/\\/g, '/');
            }
            next();
        } catch (err) {
...
async function saveChatImage(base64String, chatFolder = 'uploads/chat_images') {
    ...
    return `/${chatFolder}/${filename}`.replace(/\\/g, '/');
}
...
async function saveChatMedia(base64String, folder = 'uploads/chat_media') {
    ...
    if (mimeType.startsWith('image/')) {
        filename = `${Date.now()}_${uuidv4()}.webp`;
        ...
        filename = `${Date.now()}_${uuidv4()}.${ext}`;
        filePath = path.join(folder, filename);
